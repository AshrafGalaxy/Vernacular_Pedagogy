...
def main():
    ...
    # Clean up any non-ASCII unicode characters and ensure valid syntax in cached tokenizer files
    search_dirs = [
                ...
                if f == "tokenization_indictrans.py":
                    ...
                    try:
                        ...
                        if "—" in content:
                            ...
                            changed = True
                        if "super().__init__() FIRST" in content and "# Call super().__init__()" not in content:
                            content = content.replace("super().__init__() FIRST", "# Call super().__init__() FIRST")
                            changed = True
                        if changed:
                            with open(fpath, "w", encoding="utf-8") as wf:
                                wf.write(content)
                            print(f"  [FIX] Sanitized {fpath}")
                    except Exception as fe:
                        print(f"  [WARN] Failed to clean {fpath}: {fe}")
    ...
    try:
        ...
        print(f"[WARN] First tokenizer load attempt encountered {te}, re-patching and retrying...")
        patch_remote_tokenizer(merged_path, auth_token=hf_token)
